...
import { getFeed, getLogged, checkLike, submitLike, submitUnLike } from "../config/DataApp";
...
const renderLikeButton = (item) => {
    ...
    if (likes.includes(item.id)) {
        return (
          <Button icon="heart" mode="text" color={"red"} labelStyle={Styles.feedActionLabel} uppercase={false} onPress={() => onUnlikeItem(userInfo.user_id, item.id)}>{!likes.includes(item.id) ? item.total_likes-1 : item.total_likes+1}</Button>
          )
    }else{
      if (item.isliked) {
        return (
          <Button icon="heart" mode="text" color={"red"} labelStyle={Styles.feedActionLabel} uppercase={false} onPress={() => onUnlikeItem(userInfo.user_id, item.id)}>{item.total_likes}</Button>
          )
        }else{

        return (
          <Button icon="heart" mode="text" color={"#959595"} labelStyle={Styles.feedActionLabel} uppercase={false} onPress={() => onLikeItem(userInfo.user_id, item.id)}>{!likes.includes(item.id) ? item.total_likes : item.total_likes-1}</Button>
          )
